...
public class ContainerExtendedInventory extends ItemContainer {
    ...
    public ContainerExtendedInventory(AbstractEntityPlayer player, IInventory inventory, int width, int height, Consumer<IInventory> onClosed) {
        super(player);
        this.inventory = inventory;
        this.onClosed = onClosed;
        int sizeX = Math.max(135, width * 17 - 1);
        this.addSlotGrid(this.inventory, 0, this.inventory.getSlotAmount(), (sizeX - width * 17 - 1) / 2, 0, width);
        int playerXStart = (sizeX - 135) / 2;
        this.addPlayerInventory(player, playerXStart, height * 17 + 8);
    }
