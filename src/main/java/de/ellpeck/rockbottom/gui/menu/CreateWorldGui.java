...
public class CreateWorldGui extends Gui {
    ...
    public void init(IGameInstance game) {
        ...
        this.nameField = new InputFieldComponent(this, this.width / 2 - 75, 32, 150, 16, true, true, false, 40, true, string -> this.updateNameAndSeed(game));
        this.components.add(this.nameField);

        this.seedField = new InputFieldComponent(this, this.width / 2 - 75, 72, 150, 16, true, true, false, 40, true, string -> this.updateNameAndSeed(game));
        this.components.add(this.seedField);

        int bottomY = this.height;
        this.components.add(new ButtonComponent(this, this.width / 2 - 82, bottomY - 30, 80, 16, () -> {
            if (!this.created) {
                this.created = true;
                ...
                gui.fadeOut(20, () -> {
                    ...
                    if (info.storyMode) {
                        BasicToast toast = new BasicToast(ResourceName.intern("gui.info"), new TranslationChatComponent(ResourceName.intern("info.new_world.title")), new TranslationChatComponent(ResourceName.intern("info.new_world")), 600) {
                            @Override
