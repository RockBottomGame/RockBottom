...
public class SmithingGui extends Gui {
    ...
    private SmithingTableTileEntity tile;
    ...
    public SmithingGui(AbstractPlayerEntity player, TileEntity tile, SmithingRecipe recipe, List<ItemInstance> inputs) {
        super(TABLE_WIDTH, TABLE_HEIGHT);
        this.backgroundTex = TEX.addSuffix(".background");

        this.player = player;
        this.tile = (SmithingTableTileEntity) tile;
        this.recipe = recipe;
        this.inputs = inputs;
    ...
    protected boolean tryEscape(IGameInstance game) {
        return player.openGuiContainer(new SmithingTableGui(player, tile), new SmithingTableContainer(player, tile));
    }
