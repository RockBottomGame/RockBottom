...
public class ItemParticle extends Particle {
    ...
    public ItemParticle(IWorld world, double x, double y, double motionX, double motionY, ItemInstance instance) {
    ...
    public void render(IGameInstance game, IAssetManager manager, IRenderer g, float x, float y, int filter) {
        if (this.renderPixel == Colors.NO_COLOR) {
            ...
            if (renderer != null) {
                ...
                this.renderPixel = TileParticle.makeRenderPixel(texture);
            }
        }
