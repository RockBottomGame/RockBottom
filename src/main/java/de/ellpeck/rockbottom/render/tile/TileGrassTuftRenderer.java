...
public class TileGrassTuftRenderer extends TileMetaRenderer<TileGrassTuft> {
    ...
    @Override
    public void renderInForeground(IGameInstance game, IAssetManager manager, IRenderer g, IWorld world, TileGrassTuft tile, TileState state, int x, int y, TileLayer layer, float renderX, float renderY, float scale, int[] light) {
        if (layer == TileLayer.MAIN) {
            this.doRender(manager, world, tile, state, x, y, renderX, renderY, scale, light);
        }
    }

    private void doRender(IAssetManager manager, IWorld world, TileGrassTuft tile, TileState state, int x, int y, float renderX, float renderY, float scale, int[] light) {
        float topX = renderX + (float) Math.sin((world.getTotalTime() + (x * 50)) / 25D % (2 * Math.PI)) * scale / 15F;
        ITexture texture = manager.getTexture(this.getTexture(tile, state.get(tile.metaProp))).getPositionalVariation(x, y);
