...
public class PlayerEntity extends AbstractPlayerEntity {
    ...
    public final BiConsumer<IInventory, Integer> invCallback = (inv, slot) -> {
        if (this.world.isServer()) {
            boolean isInv = inv instanceof PlayerInventory;
            ...
            if (container != null) {
                ...
                if (index >= 0) {
                    this.sendPacket(new ContainerChangePacket(isInv, index, inv.get(slot)));
                }

                if (isInv && slot == this.getSelectedSlot()) {
                    RockBottomAPI.getNet().sendToAllPlayersExcept(this.world, new ActiveItemPacket(this.getUniqueId(), slot, this.inv.get(slot)), this);
                }
            }
        }
    };
        ...
        if (RockBottomAPI.getEventHandler().fireEvent(event) != EventResult.CANCELLED) {
            ...
            if (this.currentContainer != null) {
                if (this.world.isClient()) {
                    ...
                    if (this.world.isServer()) {
                        this.sendPacket(new ContainerDataPacket(this.currentContainer));
                    }
                }
    ...
    public void update(IGameInstance game) {
        ...
        if (!this.world.isClient()) {
            if (this.isDead()) {
                ...
                List<AbstractItemEntity> entities = this.world.getEntities(this.currentBounds.copy().expand(this.getPickupRange()), AbstractItemEntity.class);
                for (AbstractItemEntity entity : entities) {
                    if (entity.canPickUp()) {
        ...
        for (SpawnBehavior<?> behavior : Registries.SPAWN_BEHAVIOR_REGISTRY.values()) {
            if (this.world.getTotalTime() % behavior.getSpawnFrequency(this.world) == 0 && behavior.isReadyToSpawn(this.world)) {
                ...
                for (int i = behavior.getSpawnTries(this.world); i > 0; i--) {
                    ...
                    AbstractPlayerEntity closest = this.world.getClosestPlayer(x, y, this);
    ...
    public void moveToChunk(IChunk newChunk) {
        ...
        if (!this.world.isClient()) {
            ...
            for (IChunk chunk : this.chunksInRange) {
                ...
                if (nowIndex < 0) {
                    List<AbstractPlayerEntity> inRange = chunk.getPlayersInRange();
                    if (inRange.contains(this)) {
                        inRange.remove(this);
                        unload++;
                    }

                    List<AbstractPlayerEntity> outOfRange = chunk.getPlayersLeftRange();
    ...
    public void setDead(boolean dead) {
        ...
        if (!this.world.isClient() && dead) {
            ...
            RockBottomAPI.getGame().getChatLog().broadcastMessage(new TextChatComponent(FormattingCode.RED.toString()).append(new TranslationChatComponent(ResourceName.intern("death.flavor." + id), this.getName())));
    ...
    public void resetAndSpawn(IGameInstance game, double x, double y) {
        ...
        if (this.world.isServer()) {
            RockBottomAPI.getNet().sendToAllPlayersInWorld(this.world, new RespawnPacket(this.getUniqueId(), x, y));
        }
    }
