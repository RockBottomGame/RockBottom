...
public class BoomerangEntity extends AbstractBoomerangEntity {
    ...
    public BoomerangEntity(IWorld world) {
        super(world);
    }
            ...
            if (coll || distSq >= this.maxDistanceSq) {
                if (this.turnedAround) {
                    if (this.item != null) {
                        ...
                        if (remain != null) {
                            AbstractItemEntity.spawn(this.world, remain, x, y, this.motionX, this.motionY);
                        }
                    }
                    this.setDead(true);
                } else {
    ...
    public void onIntersectWithEntity(Entity otherEntity, BoundingBox thisBox, BoundingBox thisBoxMotion, BoundingBox otherBox, BoundingBox otherBoxMotion) {
        if (!this.world.isClient() && this.item != null && !this.isDead()) {
            if (otherEntity instanceof AbstractPlayerEntity) {
                if (this.turnedAround) {
                    ItemInstance remain = this.item.getItem().takeDamage(this.item, 1);
                    if (remain != null) {
                        remain = ((AbstractPlayerEntity) otherEntity).getInv().add(remain, false);
                        if (remain == null) {
                            this.setDead(true);
                        }
                    } else {
