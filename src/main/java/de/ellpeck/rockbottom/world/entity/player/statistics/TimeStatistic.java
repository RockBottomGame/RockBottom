...
public class TimeStatistic extends NumberStatistic {
    ...
    public List<StatisticComponent> getDisplayComponents(IGameInstance game, Stat stat, AbstractStatGui gui, MenuComponent menu) {
        return Collections.singletonList(new StatisticComponent(gui, () -> game.getAssetManager().localize(this.getName().addPrefix("stat.")), () -> String.format("%02d:%02d", stat.getValue() / 60, stat.getValue() % 60), stat.getValue(), null) {
            @Override
